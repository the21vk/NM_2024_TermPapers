program rtpreset2dpot
implicit none
integer::i,j
real::gamm,vel,dt,reset_rate,time_count,initial_dir,rad_square,alpha
...
time_count=0
do j=1,10000
time_count=time_count+(run(x_initial, gamm, vel, dt, initial_dir, centre, reset_rate,rad_square,alpha)/10000)
end do
print*,time_count
avg_time(i)=time_count
end do
open(11,file="rtp_2d_pot pos_09.dat")
...
close(11)
contains
function run(x_initial, gamm, vel, dt, initial_dir, centre, reset_rate,rad_square,alpha) result(time)
    real :: gamm, vel, dt, reset_rate,alpha
    real :: time,random1
    ...
    do while (flag == 0)
        ...
        xnew(1)=x(1)+COS(direction)*vel*dt-alpha*x(1)*dt
        xnew(2)=x(2)+SIN(direction)*vel*dt-alpha*x(2)*dt

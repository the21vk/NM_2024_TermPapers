...
real::x_initial,gamm,vel,dt,reset_rate,target_pos,time_count,alpha
...
time_count=0
do j=1,10000
time_count=time_count+(run(x_initial, gamm, vel, dt, initial_dir, target_pos, reset_rate,alpha)/10000)
end do
print*,time_count
avg_time(i)=time_count
end do
open(1,file="rtp_pot_pos_55.dat")
...
close(1)
contains
function run(x_initial, gamm, vel, dt, initial_dir, target_pos, reset_rate,alpha) result(time)
    real :: x_initial, gamm, vel, dt, target_pos, reset_rate
    real :: time
    real :: switch_time, reset_time, x, xnew,alpha
    ...
    do while (flag == 0)
        ...
        xnew = x-alpha*x*dt+ direction * vel * dt
